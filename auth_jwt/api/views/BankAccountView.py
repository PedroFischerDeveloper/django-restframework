...
from api.services.BankAccountService import BankAccountService
...
class BankAccountView(APIView):  
    ...
    def get(self, request, id=None):
        if id:
            bank_account = BankAccountService.get(id)
            if bank_account:
                return Response(BankAccountSerializer(data=bank_account).data, status=status.HTTP_200_OK)
            return Response(ErrorMessages.get_error('NOT_FOUND'), status=status.HTTP_404_NOT_FOUND)
        else:
            page = request.query_params.get('page', 1)
            page_size = request.query_params.get('page_size', 10)
            paged_result = BankAccountService.list_paged(page, page_size)
            ...
            if not paged_result.object_list:
                ...
                data = {
                    ...
                    "data": BankAccountSerializer(paged_result.object_list, many=True).data
                }
